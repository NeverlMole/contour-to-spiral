...
with open(file_name2, 'r') as f:
    ...
    for line in lines:
        words = line.split(' ')
        if words[0] == 'closed':
            tmp = np.zeros((int(words[1]), 3))
            cnt = int(words[1])
            continue
        if len(words) == 1:
            tmp = np.zeros((int(words[0]), 3))
            cnt = int(words[0])
        if idx < cnt:
            tmp[idx] = words
            idx += 1
...
for j in range(len(data)):
    ...
    for i in range(len(data[j])):
        ...
        x += 1
        if i != len(data[j])-1:
            print_sub(data[j][i][0], data[j][i][1], data[j][i][2],
                      data[j][i+1][0], data[j][i+1][1], data[j][i+1][2], c)
            pass
        else:
            #print_sub(data[j][i][0], data[j][i][1], data[j][i][2],
            #          data[j][0][0], data[j][0][1], data[j][0][2], c)
            pass
...
# python ./plot/plot.py ./data/test-0.txt ./data/test-0.txt-r-a0.0.spiral
